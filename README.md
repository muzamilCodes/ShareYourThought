...
Every single page and subsystem on the frontend maps directly to specific backend endpoints, controllers, and MongoDB collections:

| # | Frontend Page / Component | Core Components Used | Backend API Endpoint Hit | Controller Invoked | Database Collections & Fields Modified | Permissions |
|---|---|---|---|---|---|---|
| **1** | `app/page.tsx`<br>*(Home Feed)* | `StoryTray.tsx`<br>`CreateThought.tsx`<br>`ThoughtCard.tsx`<br>`DailyThoughtSpotlight.tsx`<br>`MobileSuggestedUsers.tsx`<br>`InstagramRightRail.tsx` | `GET /api/thoughts?sort=...`<br>`GET /api/thoughts/featured/daily`<br>`GET /api/thoughts/stories/active`<br>`GET /api/users/suggested`<br>`GET /api/categories` | `thoughtController.getThoughts`<br>`thoughtController.getDailyFeaturedThought`<br>`thoughtController.getStories`<br>`userController.getSuggestedUsers` | Reads `Thoughts` (excludes `isStory: true`), `Users`, `Categories`. Updates `viewsCount` on view. | **Public / User** |
| **2** | `app/create/page.tsx`<br>*(Create Post/Story/Draft)* | `CreateThought.tsx`<br>`AiAssistantToolbar.tsx`<br>`imageUtils.ts` (Canvas) | `POST /api/thoughts`<br>`GET /api/drafts`<br>`POST /api/drafts`<br>`DELETE /api/drafts/:id`<br>`POST /api/ai/assist`<br>`GET /api/categories` | `thoughtController.createThought`<br>`draftController.getDrafts`<br>`draftController.createDraft`<br>`draftController.deleteDraft`<br>`aiController.assistThought` | Creates document in `Thoughts` (`isStory`, `storyExpiresAt`, `gradient`, `imageUrl`) or `Drafts`. Increments `thoughtCount` in `Categories`. | **Logged-In User** |
| **3** | `app/thought/[id]/page.tsx`<br>*(Thought Detail)* | `ThoughtCard.tsx`<br>`ReportModal.tsx`<br>`CommentItem.tsx` | `GET /api/thoughts/:id`<br>`GET /api/comments/thoughts/:id`<br>`POST /api/comments/thoughts/:id`<br>`POST /api/likes/thoughts/:id`<br>`POST /api/reports` | `thoughtController.getThought`<br>`commentController.getComments`<br>`commentController.createComment`<br>`likeController.toggleLikeThought`<br>`reportController.createReport` | Reads `Thoughts`. Creates `Comments` (with `parentCommentId` for threaded replies). Updates `likes` array and creates `Notifications` and `Reports`. | **Public / User** |
| **4** | `app/profile/[username]/page.tsx`<br>*(Profile & Follow List)* | `ProfileCard.tsx`<br>`ThoughtCard.tsx`<br>`FollowersModal.tsx` | `GET /api/users/:username`<br>`POST /api/follows/:userId`<br>`GET /api/users/:username/followers`<br>`GET /api/users/:username/following`<br>`GET /api/users/me/saved` | `userController.getProfile`<br>`followController.toggleFollowUser`<br>`followController.getFollowers`<br>`followController.getFollowing` | Reads `Users` & `Thoughts`. Updates `followers`, `following`, or `followRequests` (if `isPrivate: true`). Creates `Notifications`. | **Public / User** |
| **5** | `app/analytics/page.tsx`<br>*(Creator Analytics Studio)* | `AnalyticsStudio`<br>`PerformanceTimeline` | `GET /api/analytics/me` | `analyticsController.getMyAnalytics` | Aggregates user's `Thoughts` totals (views, likes, saves, shares, comments, engagement rate) and daily trend history. | **Logged-In User** |
| **6** | `app/explore/page.tsx`<br>*(Explore Topics)* | `ThoughtCard.tsx`<br>`SectionHeading.tsx` | `GET /api/thoughts/explore/all`<br>`GET /api/thoughts/trending/top`<br>`GET /api/categories` | `thoughtController.getExploreThoughts`<br>`thoughtController.getTrendingThoughts` | Reads `Thoughts` ranked by gravity score ($(\text{Engagement})/(\text{Age}+2)^{1.3}$) and `Categories`. | **Public / User** |
| **7** | `app/search/page.tsx`<br>*(Universal Search & Hashtags)* | `SearchClient.tsx`<br>`ThoughtCard.tsx` | `GET /api/search/search/all?q=...`<br>`GET /api/hashtags/trending`<br>`GET /api/hashtags/:tag` | `hashtagController.universalSearch`<br>`hashtagController.getTrendingHashtags`<br>`hashtagController.getHashtagThoughts` | Multi-field search over `Thoughts`, `Users`, `Hashtags`, and `Categories`. | **Public / User** |
| **8** | `app/category/[slug]/page.tsx`<br>*(Category Stream)* | `ThoughtCard.tsx` | `GET /api/thoughts/category/:slug` | `thoughtController.getThoughtByCategory` | Queries `Thoughts` where `category == slug` and `isStory != true`. | **Public / User** |
| **9** | `app/notifications/page.tsx`<br>*(Notification Inbox)* | `NotificationItem.tsx` | `GET /api/notifications`<br>`PATCH /api/notifications/read-all`<br>`PATCH /api/notifications/:id/read`<br>`DELETE /api/notifications/:id` | `notificationController.getNotifications`<br>`notificationController.markAllAsRead`<br>`notificationController.markOneRead`<br>`notificationController.deleteNotification` | Reads and updates `Notifications` where `recipient == user._id` (`read: true`). Filter by like, comment, follow, message. | **Logged-In User** |
| **10**| `app/messages/page.tsx`<br>*(Direct Chat & Status)* | `ChatBox.tsx`<br>`ConversationList.tsx` | `GET /api/messages/conversations`<br>`GET /api/messages/:userId`<br>`POST /api/messages`<br>`DELETE /api/messages/:id`<br>`GET /api/messages/unread-count` | `messageController.getConversations`<br>`messageController.getMessages`<br>`messageController.sendMessage`<br>`messageController.deleteMessage` | Queries and inserts documents into `Messages` (`sender`, `recipient`, `content`, `status: 'seen'`, `seenAt`). | **Logged-In User** |
| **11**| `app/bookmarks/page.tsx`<br>*(Saved Bookmarks)* | `ThoughtCard.tsx` | `GET /api/users/me/saved` | `userController.getSavedThoughts` | Queries `Users.savedThoughts` and populates matching `Thoughts`. | **Logged-In User** |
| **12**| `app/settings/page.tsx`<br>*(Account Settings)* | `SettingsForm.tsx` | `PATCH /api/users/profile`<br>`DELETE /api/users/me` | `userController.updateProfile`<br>`userController.deleteAccount` | Updates `Users` (`name`, `bio`, `avatar`, `isPrivate`). Cascades deletes of `Thoughts` and `Comments` on account deletion. | **Logged-In User** |
| **13**| `app/admin/page.tsx`<br>*(Admin Command & Moderation)* | `SocialActivityTimeline.tsx`<br>`AdminStatsCard.tsx`<br>`UserManagementTable.tsx`<br>`ReportsQueue.tsx` | `GET /api/admin/stats`<br>`GET /api/admin/users`<br>`PATCH /api/admin/users/:id/role`<br>`DELETE /api/admin/users/:id`<br>`GET /api/reports/admin`<br>`PATCH /api/reports/admin/:id`<br>`PATCH /api/thoughts/:id/feature` | `adminController.getAdminStats`<br>`adminController.listUsers`<br>`adminController.updateUserRole`<br>`adminController.deleteUser`<br>`reportController.getAdminReports`<br>`reportController.updateReportStatus`<br>`thoughtController.toggleFeatureThought` | Aggregates all collections (`Users`, `Thoughts`, `Comments`, `Categories`, `Reports`). Moderates reported content and manages creator roles. | **Admin Only (`role: 'admin'`)** |
| **14**| `app/login/page.tsx`<br>*(Login Portal)* | `AuthForm.tsx` | `POST /api/auth/login/send-otp`<br>`POST /api/auth/login/verify-otp`<br>`POST /api/auth/login` | `authController.sendLoginOtp`<br>`authController.verifyLoginOtp`<br>`authController.login` | Queries `Users`, creates/verifies `OtpTokens`, signs and returns signed JWT token. | **Public / Guest** |
| **15**| `app/register/page.tsx`<br>*(Registration)* | `AuthForm.tsx` | `POST /api/auth/register/send-otp`<br>`POST /api/auth/register/verify-otp` | `authController.sendRegisterOtp`<br>`authController.verifyRegisterOtp` | Validates uniqueness in `Users`, creates/verifies `OtpTokens`, inserts new `User`, returns signed JWT. | **Public / Guest** |
| **16**| `app/forgot-password/page.tsx`<br>*(Password Reset)* | `AuthForm.tsx` | `POST /api/auth/forgot-password`<br>`POST /api/auth/reset-password` | `authController.forgotPassword`<br>`authController.resetPassword` | Validates email, creates `OtpTokens`, updates hashed `password` in `Users`. | **Public / Guest** |
| **17**| `app/stats/page.tsx`<br>*(Public Platform Stats)* | `TradingTimelineChart.tsx` | `GET /api/admin/stats`<br>`GET /api/health` | `adminController.getAdminStats` | Aggregates counts: `totalThoughts`, `totalUsers`, `totalViews`, `totalLikes`, `totalComments`, and live backend latency. | **Public / User** |
| **18**| `app/trending/page.tsx`<br>*(Trending Streams)* | `ThoughtCard.tsx` | `GET /api/thoughts?sort=trending` | `thoughtController.getThoughts` | Queries top thoughts ranked by 24h likes, comments, and views. | **Public / User** |
| **19**| `app/offline/page.tsx`<br>*(PWA Offline)* | `OfflineState.tsx` | *None (Client Cache)* | *Service Worker (`sw.js`)* | Served automatically by `public/sw.js` when device loses internet connection. | **Public / All** |
